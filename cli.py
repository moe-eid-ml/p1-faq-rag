...
def ground_truth_ids(item, includes=None, excludes=None):
    ...
    for i, d in enumerate(app.docs):
        ...
        if not file_ok(d["path"], includes, excludes):
            continue
        text = d["text"].lower()
        # ANY keyword match (GT is non-empty)
...
def main():
    ...
    ap.add_argument("--mode", choices=["tfidf", "semantic"], default="semantic")
    ap.add_argument("-k", type=int, default=5)
    ap.add_argument("--file", default="data/eval.jsonl")
    ap.add_argument("--both", action="store_true")
    ap.add_argument("--include", action="append", help="filename substring to include (repeatable)")
    ap.add_argument("--exclude", action="append", help="filename substring to exclude (repeatable)")
    args = ap.parse_args()
